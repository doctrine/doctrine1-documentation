...
structure, see [http://en.wikipedia.org/wiki/Tree\_data\_structure
here].
...
chapter [doc behaviors :index :name]. These templates add some
...
proper indention.
    ...
    **CAUTION** You must never assign values to ``lft``, ``rgt``,
...
all about two classes: ``Doctrine\_Tree_NestedSet`` and
``Doctrine\_Node_NestedSet``. These are nested set implementations of
the interfaces ``Doctrine\_Tree_Interface`` and
``Doctrine\_Node_Interface``. Tree objects are bound to your table
...
$category->getNode()->delete();
    ...
    **CAUTION** The above code calls ``$category->delete()`` internally.
...
$category->getNode()->isRoot();
    ...
    **NOTE** The above used functions return true/false depending on
...
$category->getNode()->getPrevSibling();
    ...
    **NOTE** The above methods return false if no next or previous
    sibling exists.
...
// ... $children = $category->getNode()->getChildren();
    ...
    **CAUTION** The ``getChildren()`` method returns only the direct
...
^^^^^^^^^^^^^^^^^^^^^^^
    ...
    **NOTE** The next example assumes you have ``hasManyRoots`` set to
...
database queries. Luckily, ``Doctrine_Query`` and some flexibility in
the nested set implementation have come to your rescue. The nested set
implementation uses ``Doctrine_Query`` objects for all it's database
work. By giving you access to the base query object of the nested set
implementation you can unleash the full power of ``Doctrine_Query``
...
You can take it even further. As mentioned in the chapter [doc
improving-performance :name] you should only fetch objects when you need
...
[doc data-fixtures :name]. Data fixtures are a great tool for loading
